...
std::unique_ptr<StandardFFTProcessor>NEWNAMEProcessor::createSpectralProcess(int index, int fftSize, int hopSize, int sampleRate, int numOverlaps, int channel, int channelCount)
{
    return std::make_unique<emptyProcess>(fftSize, hopSize, hopSize * (index%numOverlaps), (int)sampleRate, this->getPhaseBuffer());
}

void NEWNAMEProcessor::prepareProcess(StandardFFTProcessor * spectralProcessor)
{
